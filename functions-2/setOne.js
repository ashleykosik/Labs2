...
// CODE HERE
const myFunction = (p1, p2) => console.log(`The first parameter is ${p1}. The second parameter is ${p2}`)
...
*/
const merge = (firstName, lastName) => console.log(`Hello my full name is ${firstName} ${lastName}.`)
...
function pricesPlusTax (prices, callback) {
    // create array with 20% tax
    for( let i = 0; i < prices.length; i++) {
       let newPrice = (prices[i] * .20) + prices[i]
...
// CODE HERE
pricesPlusTax(prices, addPrices)
...
// adds in the value for x
const addFive = multiplyingFactory(5)
const addTen = multiplyingFactory(1)
// add in the value for y
addFive(5)
addFive(1)
