...
// CODE HERE
each(names, (index, item) => console.log(`The item at index ${index} is ${item}`))
...
// // CODE HERE 
function getUserById(arr, n, callback) {
  //search by id - print out object
  for(let i = 0; i < arr.length; i++) {
    for (let i in arr) {
      if (arr.id === n) {
        callback(arr[i])
...
getUserById(users, `15a`, (obj) => console.log(obj))
