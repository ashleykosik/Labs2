...
/* 
    Use forEach method to loop though all the elements in the 'numbers' array above, 
    and divide each element (number) by 2. Print the result in console.
*/
...
// CODE HERE
const collection = cities.filter((place) => place[place.length-1]  === 'i')
...
fruits.forEach((f, idx) => {
  if (f.color === 'red') {
    console.log(`The fruit with index ${idx} is an apple`)
  } else if (f.color === 'orange') {
    console.log(`The fruit with index ${idx} is an orange.`)
  } else {
    console.log(`The fruit with index ${idx} is neither apple or orange.`)
  }
})
...
// CODE HERE
const riceFood = foods.filter((f) => f.tags.includes('rice'))
console.log(riceFood)
const totalPrice = riceFood.reduce((a, c) => a + c.price, 0)
